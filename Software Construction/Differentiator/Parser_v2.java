...
public class Parser_v2 {
    ...
    public Expression parse() {
        ...
        if (tokenValue.equals(Token.Type.VARIABLE)){
...
//        }
        ...
        if (tokenValue.equals(Token.Type.OPEN_PARENS)){
            ...
            if (nextTokenValue.equals(Token.Type.NUMBER) || nextTokenValue.equals(Token.Type.VARIABLE)){
                Expression left = parse();
                Token.Type operator = seeNext().getType();
                if (operator.equals(Token.Type.MULTIPLICATION) || operator.equals(Token.Type.ADDITION)){
                    Token operand = getNext();
                    // Right hand side of the expression
                    Expression right = parse();
                    Token afterRight = getNext();
                    
                    if (!(afterRight.getType().equals(Token.Type.CLOSE_PARENS) || afterRight.getType().equals(Token.Type.END))) throw new InvalidInputException("Missing close parens or too many");
                    
                    // Have not reached end, continue to parse
                    if (afterRight.equals(Token.Type.CLOSE_PARENS)){
                        if (operand.getType().equals(Token.Type.MULTIPLICATION)){
                            return multiplier(left, right);
                        }
                        else {return adder(left, right);}
                        }
                    // Have reached end
                    else{
                        //if (balance == 0){
                ...
                if (operator.equals(Token.Type.CLOSE_PARENS)){
                    //exp = left;
                    return left;
...
//        }
    ...
    /**
     * Creates a new multiplication expression with this.left and this.right as arguments
     * @return Multiplication expression
     */
    public Expression multiplier(Expression left, Expression right){
        return new Multiplication(left, right);
    }
    
    /**
     * Creates a new addition expression with this.left and this.right as arguments
     * @return Addition expression
     */
    public Expression adder(Expression left, Expression right){
